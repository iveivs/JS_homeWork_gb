...
sortSelect.onchange = function () {
    ...
    if(selectedOption === 'asc') {
        productItems.sort((a,b) => {
            const priceA = parseInt(a.textContent.split('- $')[1]) // извлечение цены из текстового содержимого
    ...
    // Обновления порядкка элементов в DOM
    productItems.forEach(item => productList.appendChild(item))
}
// - - - - - - - - - - - - - - - - - - - - - - -
// Фокусировка

const inputEmail = document.forms.superForm4.input
const error = document.querySelector('#error')

inputEmail.onblur = function() {
    if( !inputEmail.value.includes('@')) {
        inputEmail.classList.add('invalid')
        error.innerHTML = 'Пожалуйста введите правильный '
    }
}
